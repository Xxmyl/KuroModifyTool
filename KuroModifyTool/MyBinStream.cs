...
namespace KuroModifyTool
{
    internal class MyBinStream
    {
        ...
        public int Add2MSB(int value, int sign)
        {
            switch (sign)
            {
                case 0: return value;
        ...
        public string IdentifyType(int value)
        {
            long removeLSB = value & 0xC0000000;
            long MSB = removeLSB >> 30;
        ...
        public byte BIdentifyType(int value)
        {
            long removeLSB = value & 0xC0000000;
            return (byte)(removeLSB >> 30);
        }

        public string GetScriptValue(byte[] buffer, int value, int off)
        {
            long removeLSB = value & 0xC0000000;
            long MSB = removeLSB >> 30;
            int actualValue = Remove2MSB(value);

            switch (MSB)
            {
                case 1: return actualValue.ToString();
                case 2:
                    actualValue = actualValue << 2;
                    float newValue = BitConverter.ToSingle(BitConverter.GetBytes(actualValue), 0);
                    return newValue.ToString();
                case 3:
                    actualValue = actualValue - off;
                    string text = DeSerialization(typeof(string), buffer, ref actualValue);
                    text = text.Replace("\n", "\\n");
                    return "\"" + text + "\"";
                default: return actualValue.ToString("x");
            }
        }

        public void SetScriptValue(string value, byte type, List<byte> modify)
        {

        }

        private uint[] LUT = {0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
        ...
        public uint ComputeCrc32(string fun_name)
        {
            byte[] buf = Encoding.ASCII.GetBytes(fun_name);
            var index_LUT = (byte)((~buf[0]) & 0xFF);
            uint result = Rec_Crc32(1, buf, LUT[index_LUT] ^ 0x00FFFFFF);
            return result;
        }
        public uint Rec_Crc32(int index, byte[] bytearray, uint crc)
        {
        ...
        public byte[] CLEPack(byte[] buf, CLEFile cfile)
        {
            byte[] output = buf;
            if (cfile.IsCompress)
            {
                output = ZstdCompressor(output);
                output = DataFill(Encoding.ASCII.GetBytes("D9BA"), output, ref cfile.CFillCount);
            }

            if (cfile.IsEncrypt)
            {
                output = BFEncrypt(output);
                output = DataFill(Encoding.ASCII.GetBytes("F9BA"), output, ref cfile.EFillCount);
            }

            return output;
        }

        public byte[] CLEUnPack(byte[] buf, CLEFile cfile)
        {
            byte[] output = buf;
            int i = 0;
            string magic = new string(DeSerialization(typeof(char[]), output, ref i, new BinStreamAttr() { Length = 4 }));
            int size = DeSerialization(typeof(int), output, ref i);
            output = ArraySub(output, i, size);
            if (magic == "F9BA" || magic == "C9BA")
            {
                cfile.IsEncrypt = true;
                output = DataRecovery(output, cfile.EFillCount);
                output = BFDecrypt(output);
                i = 0;
                magic = new string(DeSerialization(typeof(char[]), output, ref i, new BinStreamAttr() { Length = 4 }));
                size = DeSerialization(typeof(int), output, ref i);
                output = ArraySub(output, i, size);
            }
            else
            {
                cfile.IsEncrypt = false;
            }

            if (magic == "D9BA")
            {
                cfile.IsCompress = true;
                output = DataRecovery(output, cfile.CFillCount);
                output = ZstdDecompressor(output);
            }
            else
            {
                cfile.IsCompress = false;
            }

            return output;
        }

        public byte[] ArrayRemove(byte[] buf, int index, int count)
        {
            List<byte> bytes = buf.ToList();
            bytes.RemoveRange(index, count);

            return bytes.ToArray();
        }

        public byte[] ArraySub(byte[] buf, int index, int size)
        {
            byte[] bytes = new byte[size];
            Array.Copy(buf, index, bytes, 0, size);

            return bytes;
        }

        public byte[] DataFill(byte[] flag, byte[] buf, ref byte count)
        {
            List<byte> output = buf.ToList();
            int filesize = output.Count;
            int a = 8 * (int)Math.Ceiling((double)filesize / 8) - filesize;
            count = (byte)a;
            for (int i = 0; i < a; i++)
            {
                output.Add(0);
            }
            filesize = output.Count;
            output.InsertRange(0, BitConverter.GetBytes(filesize));
            output.InsertRange(0, flag);

            return output.ToArray();
        }

        public byte[] DataRecovery(byte[] buf, byte count)
        {
            return ArrayRemove(buf, buf.Length - count, count);
        }

        public byte[] BFEncrypt(byte[] buf)
        {
            ...
            string ivHex = "9D8F9DA14960CC4C";
            crypt.SetEncodedIV(ivHex, "hex");

            string keyHex = "164B7D0F4FA74CACD37A06D9F86D2094";
            crypt.SetEncodedKey(keyHex, "hex");

            return crypt.EncryptBytes(buf);
        }

        public byte[] BFDecrypt(byte[] buf)
        {
            Crypt2 decrypt = new Crypt2();
            ...
            return decrypt.DecryptBytes(buf);
        }

        public byte[] ZstdCompressor(byte[] buf)
        {
            IReadOnlyDictionary<ZSTD_cParameter, int> pairs =
                new Dictionary<ZSTD_cParameter, int>()
                {
                    {ZSTD_cParameter.ZSTD_c_checksumFlag, 1}
                };
            Compressor comp = new Compressor(new CompressionOptions(null,
                advancedParams: pairs, 
                compressionLevel: 9
                ));
            return comp.Wrap(buf);
        }

        public byte[] ZstdDecompressor(byte[] buf)
        {
            Decompressor decomp = new Decompressor();
            return decomp.Unwrap(buf);
        }
    }
    ...
    public class CLEFile
    {
        public string Name;
        public bool IsCompress;
        public bool IsEncrypt;
        public byte CFillCount;
        public byte EFillCount;
    }
}
